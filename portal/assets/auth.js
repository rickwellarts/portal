...
const FIREBASE_CONFIG = {
  ...
  storageBucket: "portal-cce.appspot.com",        // ✅ correto
...
/* ========= Guard 100% Firebase (sem fallback local) ========= */
export async function requireAuthFirebase() {
  const { firebaseOnce } = await import('./firebase.js');
  const { auth } = await firebaseOnce();
  const { onAuthStateChanged } = await import('https://www.gstatic.com/firebasejs/10.12.2/firebase-auth.js');

  const loginUrl = `${toRootPath()}index.html`;
...
// procura usuário por matrícula / login / email (legado)
...
const fb = {
  ...
  async init(){
    ...
    // ✅ evita erro "app/duplicate-app"
    const apps = getApps();
    this.app = apps.length ? apps[0] : initializeApp(FIREBASE_CONFIG);
